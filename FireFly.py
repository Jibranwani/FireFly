...
try:
    if os.getuid() != 0:
        print("\n{}[-]{}ERROR: FireFly requires root privillege to run{}".format(YELLOW,RED,GREEN))
        os._exit(1)
except:
    # Its a windows system
    print("\n{}[-]{}ERROR: FireFly Cannot run on Windows.{}".format(YELLOW,RED,GREEN))
    sleep(1.5)
    os._exit(1)
...
sys.stdout.write(GREEN + spaces + """
    ...
    """ + END + BLUE +
    '\n' + '{}Detects ARP Spoofing Attacks!{}'.format(RED,BLUE).center(98) +
...
if choice == '1':
    os.system("clear")
    print("{}[-]{}FireFly Started. Any output will be redirected to log file.".format(YELLOW,BLUE))
    sniff(filter="arp",prn=process_packets, store=0 )
elif choice =='2':
    print("{}[-]{}Exiting FireFly.").format(YELLOW,RED)
    sleep(1.5)
